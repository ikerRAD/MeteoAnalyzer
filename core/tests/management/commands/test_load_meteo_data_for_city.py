...
from core.domain.exceptions.city_already_exists_exception import CityAlreadyExistsException
...
from core.infrastructure.persistence.repositories.db_city_repository import DbCityRepository
from core.infrastructure.persistence.repositories.db_weather_data_repository import DbWeatherDataRepository
...
class TestCommand(unittest.TestCase):
    ...
    def test_handle_invalid_date_range(self) -> None:
        with self.assertRaisesRegex(ValueError, "2024-01-02 happens after 2024-01-01"):
            self.command.handle(city_name="Madrid", start_date="2024-01-02", end_date="2024-01-01", strategy="first")

    def test_handle_index_strategy_without_index(self) -> None:
        with self.assertRaisesRegex(ValueError, "strategy by-index requires --index"):
            self.command.handle(city_name="Madrid", start_date="2024-01-01", end_date="2024-01-01", strategy="by-index")

    def test_handle_index_strategy_with_negative_index(self) -> None:
        with self.assertRaisesRegex(ValueError, "--index is required to be greater or equal to 0!"):
            self.command.handle(city_name="Madrid", start_date="2024-01-01", end_date="2024-01-01", strategy="by-index",
                                index=-1)
    ...
    def test_handle_city_not_found(self) -> None:
        ...
        self.command.handle(city_name="Nonexistent City", start_date="2024-01-01", end_date="2024-01-01", strategy="first")

        self.command.stdout.write.assert_called_once_with("No cities found with the name Nonexistent City, exiting...")
        self.meteo_client.get_cities_by_name.assert_called_once_with("Nonexistent City")

    def test_handle_http_error(self) -> None:
        self.meteo_client.get_cities_by_name.side_effect = HTTPError(response=Mock(spec=Response, status_code=500))

        self.command.handle(city_name="Barcelona", start_date="2024-01-01", end_date="2024-01-01", strategy="first")

        self.command.stderr.write.assert_called_once_with(f"Error retrieving 'Barcelona' from external API. status code: 500")
    ...
    def test_handle_first_strategy(self, process_one_city: Mock) -> None:
        self.command.handle(city_name="Madrid", start_date="2024-01-01", end_date="2024-01-01", strategy="first")

        process_one_city.assert_called_once_with(self.madrid_city_1, "2024-01-01", "2024-01-01")
    ...
    def test_handle_all_strategy(self, process_all_cities: Mock) -> None:
        self.command.handle(city_name="Madrid", start_date="2024-01-01", end_date="2024-01-01", strategy="all")

        process_all_cities.assert_called_once_with(self.madrid_cities, "2024-01-01", "2024-01-01")
        self.meteo_client.get_cities_by_name.assert_called_once_with("Madrid")
    ...
    @patch(
        'core.management.commands.load_meteo_data_for_city.Command._Command__process_one_city')
    def test_handle_select_strategy_with_valid_input(self, process_one_city: Mock, *_) -> None:
        self.command.handle(city_name="Madrid", start_date="2024-01-01", end_date="2024-01-01", strategy="select")

        process_one_city.assert_called_once_with(self.madrid_city_2, "2024-01-01", "2024-01-01")
        self.meteo_client.get_cities_by_name.assert_called_once_with("Madrid")
    ...
    def test_handle_select_strategy_with_input_out_of_bounds(self, *_) -> None:
        ...
        self.meteo_client.get_cities_by_name.assert_called_once_with("Madrid")
        self.command.stderr.write.assert_called_once_with("Invalid input! It had to be a number within the specified bounds!")
    ...
    def test_handle_index_strategy(self, process_one_city: Mock) -> None:
        self.command.handle(city_name="Madrid", start_date="2024-01-01", end_date="2024-01-01", strategy="by-index", index=1)
    ...
    def test_handle_index_strategy_index_out_of_bounds(self) -> None:
        ...
        self.command.handle(city_name="Barcelona", start_date="2024-01-01", end_date="2024-01-01", strategy="by-index", index=1)
    ...
    def test_process_one_city_success(self) -> None:
        ...
        self.command._Command__process_one_city(self.barcelona_city, "2024-01-01", "2024-01-01")
        ...
        self.command.stdout.write.assert_called_with("City 'Barcelona' [41.3851, 2.1734] weather data successfully inserted from 2024-01-01 to 2024-01-01!")
    ...
    def test_process_one_city_no_data(self) -> None:
        ...
        self.command.stdout.write.assert_called_with("No weather data for city 'Barcelona' [41.3851, 2.1734] found in the specified dates!")
    ...
    def test_process_one_city_already_exists(self) -> None:
        ...
        self.command.stdout.write.assert_has_calls([
            call("City 'Barcelona' already exists in the database. Skipping insert..."),
            call("City 'Barcelona' [41.3851, 2.1734] weather data successfully inserted from 2024-01-01 to 2024-01-01!")
        ])
    ...
    def test_process_all_cities(self, process_one_city) -> None:
        self.command._Command__process_all_cities(self.madrid_cities, "2024-01-01", "2024-01-01")
