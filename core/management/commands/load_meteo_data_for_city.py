...
class Command(BaseCommand):
    ...
    def __init__(self, meteo_client: MeteoClient | None = None, city_repository: CityRepository | None = None, weather_data_repository: WeatherDataRepository | None = None):
        ...
        self.__weather_data_repository = weather_data_repository or DbWeatherDataRepositoryFactory.create()

    def add_arguments(self, parser) -> None:
        parser.add_argument("city_name", type=str, help="The name of the city to load data for.")
        parser.add_argument(
            'start_date',
            type=str,
        ...
        parser.add_argument(
            ...
            help='Action to take if multiple cities are found. "first" (default), "all", "select" (interactive), or "by-index" (requires --index).'
        )
        parser.add_argument('--index', type=int, help='Index of the city to select when --action is "by-index".')
    ...
    def handle(self, *args, **options) -> None:
        ...
        if strategy == "by-index":
            if index is None:
                raise ValueError("strategy by-index requires --index")
            elif index < 0:
        ...
        try:
            ...
            self.stderr.write(f"Error retrieving '{city_name}' from external API. status code: {error.response.status_code}")
            return
        ...
        if strategy == "by-index" and index >= len(retrieved_cities):
            self.stderr.write(f"There are not enough retrieved cities for the index '{index}'... "
                         f"Remember that the index must go from 0 to N-1 being N the number of cities retrieved: {len(retrieved_cities)}")
            return
    ...
    def __process_input(self, cities: list[City], start_date: str, end_date: str) -> None:
        ...
        try:
            ...
            self.stderr.write("Invalid input! It had to be a number within the specified bounds!")
    ...
    def __process_one_city(self, city: City, start_date: str, end_date: str) -> None:
        try:
            ...
            self.stdout.write(f"City '{city.name}' already exists in the database. Skipping insert...")
            city.id = error.city_id
            saved_city = city

        weather_data_list = self.__meteo_client.get_weather_data_by_city(saved_city, start_date, end_date)

        if len(weather_data_list) == 0:
            self.stdout.write(f"No weather data for city '{saved_city.name}' [{saved_city.latitude}, {saved_city.longitude}] found in the specified dates!")
            return

        self.__weather_data_repository.bulk_save(weather_data_list)
        self.stdout.write(f"City '{saved_city.name}' [{saved_city.latitude}, {saved_city.longitude}] weather data successfully inserted from {start_date} to {end_date}!")


    def __process_all_cities(self, cities: list[City], start_date: str, end_date: str) -> None:
